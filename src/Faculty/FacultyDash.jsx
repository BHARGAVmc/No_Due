...
function FacultyDash() {
  ...
  // ✅ Add button click handler (navigates to FacultyIn page)
  const handleAddClick = () => {
    window.location.href = 'FacultyIn'; // or 'FacultyIn.html' if needed
  };
              ...
              <ul>
                {dep.years.map((yearObj, yIndex) =>
                  yearObj.sections.map((section, secIndex) => (
                    <li
                      key={`${yIndex}-${secIndex}`}
                      onClick={() => handleClassClick(dep.name, yearObj.year, section)}
                    >
